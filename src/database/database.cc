...
public:
    BuriedDbImpl(const std::string& path) 
      :db_path_(path) {
        storage_ = std::make_unique<DBStorage>(InitStorage(path));
    ...
    std::vector<BuriedDb::Data> QueryData(uint32_t limit_size) {
...
void BuriedDb::InsertData(const BuriedDb::Data& data) {
    impl_->InsertData(data);
}

void BuriedDb::DeleteData(const BuriedDb::Data& data) {
    impl_->DeleteData(data);
}

void BuriedDb::DeleteDatas(const std::vector<BuriedDb::Data>& datas) {
    impl_->DeleteDatas(datas);
}

std::vector<BuriedDb::Data> BuriedDb::QueryData(int32_t limit_size) {
    return impl_->QueryData(limit_size);
}
